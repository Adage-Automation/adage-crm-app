...
function LeadDetailPanel({ lead, onClose }) {
  ...
  const statusBadgeColor = STATUS_COLORS[statusVal] || T.textMuted;
    ...
    <div style={{ marginTop: 6, border: `1px solid ${T.border}`, borderRadius: 10, background: T.bgCard, overflow: "hidden", animation: "fadeIn 0.2s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 16px", borderBottom: `1px solid ${T.border}`, background: T.bgCardAlt }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {statusVal && (
            <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 100, background: `${statusBadgeColor}20`, color: statusBadgeColor }}>{statusVal}</span>
          )}
        </div>
...
function RevenueDonut({ leads, onSegmentClick, activeKey }) {
  ...
  leads.forEach(l => {
    ...
    if (!s) return;
    if (!byStatus[s]) byStatus[s] = { rev: 0, count: 0, color: STATUS_COLORS[s] || T.accent };
...
function ListRow({ lead, isSelected, onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display: "grid", gridTemplateColumns: LIST_GRID, gap: 8, padding: "10px 14px", alignItems: "center", borderBottom: `1px solid ${T.border}`, borderLeft: `4px solid ${urg.border}`, background: isSelected ? T.accentBg : hovered ? T.bgCardAlt : "transparent", cursor: "pointer", transition: "background 0.15s" }}>
      <div>
...
// ─── Kanban card ──────────────────────────────────────────────────────────────
function KanbanCard({ lead, isSelected, onClick }) {
  const [hovered, setHovered] = useState(false);
  const urg = URGENCY[getUrgency(lead.x_studio_expected_closing)];
  const closingDate = fmtShort(lead.x_studio_expected_closing);
  const regionColor = REGION_COLORS[lead.x_studio_responsible_region_1] || T.textMuted;
  return (
    <div onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ background: isSelected ? T.accentBg : hovered ? T.bgCardAlt : T.bgCard, border: `1px solid ${isSelected ? T.accent : T.border}`, borderLeft: `4px solid ${urg.border}`, borderRadius: 10, padding: "12px 14px", cursor: "pointer", transition: "all 0.15s", marginBottom: 8 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: T.textPrimary, marginBottom: 3, lineHeight: 1.3 }}>{lead.name}</div>
      <div style={{ fontSize: 11, color: T.textMuted, marginBottom: 8 }}>{lead.partner_id?.[1] || lead.partner_name || "—"}</div>
      <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 8 }}>
        {lead.x_studio_project_background && (
          <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 100, background: getProjectTypePill(lead.x_studio_project_background).bg, color: getProjectTypePill(lead.x_studio_project_background).color }}>{lead.x_studio_project_background}</span>
        )}
        {lead.x_studio_responsible_region_1 && (
          <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 100, background: `${regionColor}18`, color: regionColor }}>{lead.x_studio_responsible_region_1}</span>
        )}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 11, color: T.textSecondary }}>{lead.x_studio_sales_lead?.[1] || "—"}</span>
        <span style={{ fontSize: 12, fontWeight: 700, color: lead.expected_revenue > 0 ? T.success : T.textMuted }}>{lead.expected_revenue > 0 ? fmt(lead.expected_revenue) : "—"}</span>
      </div>
      <div style={{ fontSize: 11, color: closingDate ? urg.dateColor : T.textMuted, marginTop: 4 }}>{closingDate || "No date"}</div>
    </div>
  );
}
...
export function PipelineTab({ leads, stages }) {
  ...
  }, [leads, activeOnly, filterRegion, filterStatus, filterPerson, filterLeadStatus, filterProjectType, selectedMonth]);
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 10, justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          ...
          {anyFilter && (
            <button onClick={() => { setFilterRegion([]); setFilterStatus([]); setFilterPerson([]); }}
      ...
      {(filterLeadStatus || filterProjectType || selectedMonth) && (
        <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {filterLeadStatus && (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 100, background: `${STATUS_COLORS[filterLeadStatus] || T.accent}20`, color: STATUS_COLORS[filterLeadStatus] || T.accent, border: `1px solid ${STATUS_COLORS[filterLeadStatus] || T.accent}40` }}>
      ...
      {groups.map(({ key, leads: groupLeads, idx }) => {
        ...
        return (
          <div key={key} style={{ marginBottom: 14 }}>
            {/* Group header — clickable in list mode to collapse */}
            <div onClick={() => viewMode === "list" && toggleCollapse(key)}
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: isCollapsed || viewMode === "kanban" ? 10 : "10px 10px 0 0", marginBottom: 0, cursor: viewMode === "list" ? "pointer" : "default", userSelect: "none" }}>
            ...
            {viewMode === "kanban" && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 14, marginTop: 10 }}>
                {groupLeads.map(lead => (
                  <div key={lead.id}>
                    <KanbanCard lead={lead} isSelected={selectedLeadId === lead.id} onClick={() => setSelectedLeadId(selectedLeadId === lead.id ? null : lead.id)} />
                    {selectedLeadId === lead.id && (
                      <LeadDetailPanel lead={lead} onClose={() => setSelectedLeadId(null)} />
                    )}
                  </div>
