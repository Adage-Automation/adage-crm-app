...
const ODOO_BASE = import.meta.env.DEV
  ? "/api/odoo"
  : import.meta.env.VITE_ODOO_URL || "https://crm-adage-5.odoo.com";
const API_KEY = import.meta.env.VITE_ODOO_API_KEY || "";

const fetchOdoo = async (model, method, args = [], kwargs = {}) => {
  const headers = { "Content-Type": "application/json" };
  if (!import.meta.env.DEV && API_KEY) headers.Authorization = `Bearer ${API_KEY}`;
  const res = await fetch(`${ODOO_BASE}/web/dataset/call_kw`, {
    method: "POST", headers,
    body: JSON.stringify({
      jsonrpc: "2.0", method: "call",
      params: { model, method, args, kwargs: { context: { lang: "en_US" }, ...kwargs } },
    }),
  });
...
const STATUS_CONFIG = {
  Planned:     { bg: "#F59E0B", text: "#fff", border: "#D97706" },  // yellow
  Completed:   { bg: "#10B981", text: "#fff", border: "#059669" },  // green
  Rescheduled: { bg: "#F97316", text: "#fff", border: "#EA580C" },  // orange
  Cancelled:   { bg: "#EF4444", text: "#fff", border: "#DC2626" },  // red
...
// ─── Detail panel ─────────────────────────────────────────────────────────────
function DetailPanel({ engagement, lead, empMap, onClose }) {
  ...
  const assignees = persons.map(p => resolveEmpName(p, empMap)).filter(Boolean).join(", ") || "—";
  ...
  const Field = ({ label, value, color, mono }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      <div style={{ fontSize: 13, color: color || T.textPrimary, fontWeight: 500, fontFamily: mono ? "monospace" : "inherit" }}>
        {value || "—"}
      </div>
    </div>
  );
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {isAnomaly && (
            <span style={{ fontSize: 11, color: "#D97706", fontWeight: 600 }}>
              ⚠ Visit recorded but status not updated
            </span>
          )}
        </div>
        <button onClick={onClose} style={{
          background: "none", border: "none", cursor: "pointer",
          fontSize: 18, color: T.textMuted, lineHeight: 1, padding: "2px 6px",
